...
- **AD backend.** On a Reactant device training differentiates with Enzyme (under Reactant's ABI) by default. Passing an explicit `adtype` such as `AutoZygote()` or `AutoMooncake()` is honoured — Flux then attempts to compile the step through that backend instead (subject to Reactant's op coverage). `Duplicated` models are not supported here — pass the plain model that already lives on the device.
