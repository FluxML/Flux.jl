...
The `freezeparam!` function prevents parameter from being updated. The following example stops the waight parameter of the second layer `Dense(2, 3)` from being updated.
...
train!(
    ...
    freezeparam!(params(m), m.layers[2].W),
    [(rand(4, 10), rand(1, 10))],
    ADAM(0.005)
)
```
