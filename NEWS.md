...
- `Flux.train_step!` and `Flux.train_step_withgradient!` now support **auxiliary loss outputs**: if the loss returns a `Tuple` or `NamedTuple` whose first element is the scalar loss, the gradient is taken of the loss alone and the whole value is returned (handy for logging metrics computed in the forward pass). `train!` accepts such losses too, guarding on the scalar. Works on every backend, including Reactant, where the scalar loss is read back to the host and the auxiliary outputs stay on the device ([#2711](https://github.com/FluxML/Flux.jl/pull/2711)).
...
- Added the public single-step training primitives `Flux.train_step!(loss, [adtype,] model, batch::Tuple, opt_state) -> loss` and `Flux.train_step_withgradient!(...) -> (loss, grad)`, which run one optimisation step (gradient + in-place optimiser update) and return the loss (and, for the latter, the gradient with respect to the model). `train!` is now a loop built on top of `train_step!`. On a Reactant device the step is compiled and cached automatically (the cached step is dropped once the model is garbage-collected) ([#2709](https://github.com/FluxML/Flux.jl/pull/2709)).
