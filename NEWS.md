...
- Added `Flux.autocast` for PyTorch-style mixed-precision training: `autocast(model, T)` wraps a model so that matmul/convolution-heavy layers cast parameters and inputs to the half-precision type `T` (`Float16`/`BFloat16`) at call time, while normalization layers compute in `Float32`. It is applied automatically via the new `autocast` keyword of `gradient`, `withgradient` and `train!`. Parameters stay `Float32` ("master weights"), gradients are accumulated back in `Float32` and shaped like the unwrapped model, so the usual optimiser setup works unchanged. Because the wrapping is a plain model transform rather than a runtime scope, both plain and wrapped forward passes keep their exact inferred element type. Custom layers opt in via `Flux.autocast_mode` ([#2702](https://github.com/FluxML/Flux.jl/pull/2702)).
- **breaking change**: the loss functions in `Flux.Losses` now always accumulate in `Float32` when given `Float16`/`BFloat16` inputs (returning a `Float32` loss), for numerical stability under mixed precision. `Float32`/`Float64` inputs are unaffected ([#2702](https://github.com/FluxML/Flux.jl/pull/2702)).
