...
- `train!` now compiles the whole training step (forward, Enzyme reverse, and optimiser update) into a single cached XLA executable when the model lives on a [Reactant](https://github.com/EnzymeAD/Reactant.jl) device. Move the model to the device, call `Flux.setup`, and pass device-resident data. Requires `Optimisers ≥ 0.4.8` ([#2707](https://github.com/FluxML/Flux.jl/pull/2707)).
- `f16` and `bf16` now convert `BatchNorm`, `InstanceNorm` and `GroupNorm` in **mixed precision**: statistics and affine parameters stay in `Float32` while data flows in half precision, so half-precision `BatchNorm` now works on the GPU. These layers' parameters are no longer downcast to `Float16`/`BFloat16`. Also adds bf16 `Conv`/pooling GPU coverage. Requires NNlib ≥ 0.9.42 ([#2700](https://github.com/FluxML/Flux.jl/pull/2700)).
- The normalization layers now delegate their forward pass to `NNlib.batchnorm`/`instancenorm`/`groupnorm`/`normalise` (requires NNlib v0.9.41). As a side effect, `LayerNorm` and `Flux.normalise` now add `eps` (rather than `eps^2`) to the variance, matching the other layers. `Flux.normalise` is now a thin wrapper around `NNlib.normalise`, which should be preferred ([#2701](https://github.com/FluxML/Flux.jl/pull/2701)).
- Removed the `FluxCUDAcuDNNExt` extension and the `cuDNN` dependency: cuDNN-accelerated `BatchNorm` is now provided by NNlib and selected automatically for `CuArray`s. On AMDGPU the MIOpen fast path was removed pending [NNlib.jl#752](https://github.com/FluxML/NNlib.jl/issues/752) ([#2701](https://github.com/FluxML/Flux.jl/pull/2701)).
- `train!` now defaults to `caching_allocator = false, gc_interval = :auto`: it drops the cross-step allocation cache and instead paces incremental GC from step timing, keeping GPU memory at the working set while bounding the reserved-memory growth of [#2523](https://github.com/FluxML/Flux.jl/issues/2523) ([#2695](https://github.com/FluxML/Flux.jl/pull/2695), [#2697](https://github.com/FluxML/Flux.jl/pull/2697)).
