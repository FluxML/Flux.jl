...
@testset "Precision" begin
  ...
  @testset "mixed-precision normalization ($cast)" for cast in (f16mix, bf16mix)
    Thalf = cast === f16mix ? Float16 : BFloat16
    full = cast === f16mix ? f16 : bf16
    # Under `f16mix`/`bf16mix`, BatchNorm/InstanceNorm/GroupNorm keep statistics and
    # affine parameters in Float32; LayerNorm and everything else is converted fully.
    for layer in (BatchNorm(3),
      ...
      # plain `f16`/`bf16` is a full cast, like PyTorch's `model.half()`
      @test all(p -> eltype(p) == Thalf,
                filter(x -> x isa AbstractArray, collect(Functors.fleaves(full(layer)))))
    end
    @test all(p -> eltype(p) == Thalf,
              filter(x -> x isa AbstractArray, collect(Functors.fleaves(cast(LayerNorm(3))))))
    # non-norm layers convert fully under the mixed cast too
    @test eltype(cast(Dense(3 => 4)).weight) == Thalf
    ...
    if cast === f16mix
      ...
      @test eltype(f16mix(BatchNorm(3))(xh)) == Float16       # mixed: Float32 params, f16 data
      @test eltype(f16mix(InstanceNorm(3; affine=true))(xh)) == Float16
      @test eltype(f16mix(GroupNorm(4, 2))(f16(randn(Float32, 4, 4, 4, 2)))) == Float16
    end
  end
