...
@testset "BatchNorm" begin
  let m = BatchNorm(2), x = param([1 3 5;
    ...
    # initial (m.running_var)^.5 is 1
    # initial m.running_mean is 0
    @test m.active
    ...
    #  2.0  4.0  6.0
    #
    # running_mean(μ) of batch will be
    #  (1. + 3. + 5.) / 3 = 3
    #  (2. + 4. + 6.) / 3 = 4
    #
    ...
    #  .1 * 3 + 0 = .3
    #  .1 * 4 + 0 = .4
    @test m.running_mean ≈ reshape([0.3, 0.4], 2, 1)

    # julia> .1 .* var(x, dims = 2, corrected=false) .* (3 / 2).+ .9 .* [1., 1.]
    # 2×1 Array{Float64,2}:
    #  1.3
    #  1.3
    @test m.running_var ≈ .1 .* var(x.data, dims = 2, corrected=false) .* (3 / 2).+ .9 .* [1., 1.]
