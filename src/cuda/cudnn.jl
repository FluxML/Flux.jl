...
function (BN::Flux.BatchNorm)(x::Union{CuArray{T,2},CuArray{T,4},CuArray{T,5}},
  ...
  BN.affine || throw(ArgumentError("BatchNorm: only affine=true supported on gpu"))
  BN.track_stats || throw(ArgumentError("BatchNorm: only track_stats=true supported on gpu"))
  length(BN.β) == size(x, ndims(x)-1) || throw(ArgumentError("BatchNorm: input has wronng number of channels"))
