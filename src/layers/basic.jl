...
function (a::Dense)(x)
  ...
  if length(in_size) <= 2
    ...
    extra_dims = in_size[2:end]
    out_feature_size = size(b)[1]

    out_size = (out_feature_size, extra_dims...)

    A = zeros(out_size)

    dim_range = Iterators.map(x -> 1:x, extra_dims)

    for d in Iterators.product(dim_range...)
      A[:, d...] = W * x[:, d...] .+ b
    end
