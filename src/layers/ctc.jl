...
function ctc(ŷ::CuArrays.CuArray, y)
  ...
  @cuda blocks=1 threads=U′ computeAlphaKernel(ŷ, length(z), size(ŷ,2), nRepeats, gpu(z), CUDAdrv.CuArray(z′), alphas, blank)
  grads = gpu([-Inf32 for x in 1:length(ŷ)])
  output = gpu([-Inf32 for x in 1:(size(ŷ,2) * U′)])
  accum = gpu([-Inf32 for x in 1:length(ŷ)])
  
  @cuda blocks=1 threads=U′ computeBetasAndGradKernel(ŷ, length(z), size(ŷ,2), nRepeats, CUDAdrv.CuArray(z′), alphas, betas, output, accum, grads, blank)
