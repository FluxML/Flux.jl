...
using NNlib
import NNlib: softmax, ∇softmax, logsoftmax, ∇logsoftmax, conv2d, conv3d, pool2d, pool3d
...
pool2d(x::TrackedArray{<:Any,4}; window = 2, mode = 0, padding = 0) =
  TrackedArray(Call(_pool2d, x, window, padding, mode))
...
back_(::typeof(_pool2d), y, Δ, x, k, pad, mode) =
  ...
  conv3d(x::TrackedArray{<:Any,5}, w::TrackedArray{<:Any,5}; stride = 1, padding = 0) =
    TrackedArray(Call(_conv3d, x, w, stride, padding))
  ...
  pool3d(x::TrackedArray{<:Any,5}; window = 2, mode = 0, padding = 0) =
    TrackedArray(Call(_pool3d, x, window, padding, mode))
